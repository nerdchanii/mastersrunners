...
while IFS= read -r file; do
  ...
  find design/architecture design/backend design/frontend design/operating-rules docs/domain \
    -maxdepth 1 -type f -name '*.md' |
    LC_ALL=C sort
)

if [ "${#doc_files[@]}" -eq 0 ]; then
  printf 'No docs found for frontmatter check.\n'
  exit 0
fi
...
if [ "${#errors[@]}" -gt 0 ]; then
  printf 'Doc frontmatter check failed.\n'
  printf 'Expected doc_state, owner, last_verified, and sources frontmatter in design foundation docs and docs/domain.\n'
  printf ' - %s\n' "${errors[@]}"
  exit 1
fi

printf 'Doc frontmatter check passed for %d file(s).\n' "${#doc_files[@]}"
