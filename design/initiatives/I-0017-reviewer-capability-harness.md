...
task 시스템은 reviewer 역할 이름을 요구하지만, 현재 저장소에는 그 역할을 실제로 수행할 수 있는 공식 `agents`/`skills` 프로토콜 기반 reviewer 정의가 없다. 이 이니셔티브는 specialist reviewer와 product review를 이름이 아니라 실제 OpenAI Codex 및 Claude Code 공식 protocol artifact로 만들고, task review 하네스를 그 protocol과 실행 증빙에 연결한다.

## 현재 결과

이 이니셔티브의 mandatory review, structured artifact gate, Codex session-end hook 방향은 `I-0025-agent-review-harness-retirement`에서 superseded됐다. 저장소는 과거 `tasks/reviews/**` 증빙은 보존하지만, review evidence artifact와 live orchestration harness는 더 이상 현재 운영 경로가 아니다.

아래 문제, 목표, 범위, 성공 기준은 당시 이니셔티브 기록으로만 남긴다. 현재 작업자는 이 문서를 live reviewer 또는 orchestration 지침으로 사용하지 않는다.

## 문제

- frontend review, backend review, UI/UX review, workflow review, docs review, product review 같은 reviewer 이름은 문서와 task frontmatter에만 존재하거나, 1차 시도에서는 `harness/capabilities/...` 같은 저장소 내부 관례에만 연결돼 있다.
...
- baseline reviewer와 product review를 OpenAI Codex 공식 `.codex/agents/`, `.agents/skills/` 및 Claude Code 공식 `.claude/agents/`, `.claude/skills/` protocol artifact로 정의한다.
...
- review evidence을 곧바로 모든 에스컬레이션 reviewer까지 확장하지 않는다.
...
- process / protocol / guardrail 정합성: workflow review
- 문서 clarity 및 사용성: docs review
...
- 저장소가 baseline reviewer와 product review를 OpenAI Codex 및 Claude Code 공식 protocol artifact로 가진다.
...
- 2026-04-04: baseline reviewer 6종과 product review를 `harness/capabilities/agents/`에 등록하고, metadata gate가 active reviewer capability inventory를 확인하도록 정리했다.
...
- 2026-04-04: `I-0017-020`에서 repo-local agent/skill files, Claude mirror files, reviewer registry, `tasks/reviews/<task-id>/<reviewer>.json`을 기준으로 review evidence과 closeout gate를 재정렬했다.
...
- 2026-04-04: `.gitignore`가 `.codex/`와 `.claude/` 전체를 막고 있어 실제 review evidence files가 git에 올라가지 않던 누락을 `I-0017-050`에서 수정하고, repo가 공식 protocol artifact 자체를 실제 버전 관리 대상으로 갖도록 정리했다.
