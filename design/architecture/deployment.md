...
- Ops web: separate static Vite SPA on Cloudflare Pages
...
- Build artifact for ops web: static files from `apps/ops-web/dist`
- Web build entrypoint: root `pnpm build:web` script
- Ops-web build entrypoint: root `pnpm build:ops-web` script
...
- Ops-web response-header source of truth: `apps/ops-web/public/_headers`
...
- The future ops host, its Access policy, dedicated Pages project, and any `/api-docs*` or operator-route proxy rules are external state tracked under `EX-0007`
