...
- 2026-09-02: `.husky/pre-push`를 harness structure 검사, `format:check`, `lint`, `run-typecheck.sh`만 돌리도록 바꾸고 `commit-conventions.md` Enforcement 절을 맞췄다. `--no-verify` 금지 문장을 추가했다. 훅 주석에 남아 있던 `pnpm ci:local` 문자열은 verify의 negative grep에 걸려 문장으로 풀어 썼다.
- 2026-09-02: 전체 `pnpm ci:local` 실행 결과. harness 구조, format, lint, UX copy, typecheck, depcruise, knip, 패키지 빌드, API 커버리지(70 suites, 835 tests), web 빌드까지 통과했고 마지막 단계인 Playwright UX contracts만 로컬에 chromium headless shell이 설치되어 있지 않아 6개가 실패했다(환경 문제, `pnpm --filter @masters/web exec playwright install chromium`으로 해결 가능). 삭제한 `profile-api.ts`와 관련된 `profile-route-query-migration.test.tsx`는 별도로 실행해 8개 모두 통과했다.
- 2026-09-02: 이 브랜치는 워크트리 격리 가드 때문에 새 워크트리 대신 `agent-harness-audit` 워크트리 안에서 `dev`(5d05bea) 기준으로 만들었다. 커밋은 config·ignore, I-0026 문서·프롬프트, ast-grep 분할, knip·pre-push, 태스크 마감 5개로 나눴다. 메인 체크아웃의 더티 파일은 이 브랜치에 커밋됐으므로 사용자가 `git checkout --`과 untracked 복사본 삭제로 정리해야 한다.
