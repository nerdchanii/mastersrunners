...
Phase 5까지 핵심 기능 구현 완료. Phase 6는 **프로덕션 배포 준비 + 미완성 UI 마무리 + 사용자 경험 강화**를 목표로 한다.

- 배포: OAuth 실연동, CI/CD 파이프라인 검증, 환경 분리
- 품질: Phase 5 잔여 항목 처리, shadcn/UI 완전 전환
- UX: 다크 모드, 이미지 뷰어, 워크아웃↔포스트 연동 흐름
...
- [ ] 카카오 OAuth 앱 등록 + 콜백 URL 설정
- [ ] 구글 OAuth 앱 등록 + 콜백 URL 설정
- [ ] 네이버 OAuth 앱 등록 + 콜백 URL 설정
- [ ] 환경별 OAuth redirect URI 분리 (dev/staging/prod)
- [ ] dev-login 엔드포인트 프로덕션 빌드에서 완전 제거 확인
...
- [ ] shadcn/UI CSS 변수 기반 다크 테마 설정
- [ ] ThemeProvider + localStorage 테마 저장
- [ ] 시스템 설정 연동 (prefers-color-scheme)
- [ ] 설정 페이지에 테마 토글 추가

### C-2. 계정 삭제/탈퇴 (M)

- [ ] DELETE /profile (soft delete + 개인정보 익명화)
- [ ] 탈퇴 확인 UI (비밀번호/OAuth 재인증)
- [ ] 관련 데이터 처리 정책 (포스트/댓글/좋아요 유지 vs 삭제)
- [ ] 30일 유예 기간 후 영구 삭제 (선택)

### C-3. 해시태그 검색 (L)

- [ ] 해시태그 파싱 + 저장 (Post 생성/수정 시 추출)
- [ ] GET /posts/hashtag/:tag 엔드포인트
- [ ] 해시태그 클릭 → 검색 결과 페이지 연동
- [ ] 인기 해시태그 목록 API
